#!/usr/bin/env python3
"""
Stack v2 Dataset Content Downloader and Preprocessor (Software Heritage)

The Stack v2 datasets on HuggingFace contain only metadata, not file content.
This script downloads the actual code content from Software Heritage Archive
using the blob_id and preprocesses it in the same format as preprocess_datasets.py.
...
- url: str (Software Heritage URL for verification)

Usage:
    # Full download
    python preprocess_stack_v2_swh.py \
...
Note: Software Heritage API has stricter rate limits than GitHub.
      Use fewer workers (2-4) to avoid being rate limited.
...
SWH_API_BASE = "https://archive.softwareheritage.org/api/1/content/sha1_git:{blob_id}/raw/"
...
def download_content_from_swh(
    blob_id: str,
    max_retries: int = 3
) -> Tuple[Optional[str], str, str]:
    """Download file content from Software Heritage Archive.

    Args:
        blob_id: Git blob SHA1 hash
        max_retries: Maximum number of retry attempts

    Returns:
        Tuple of (file_content, error_type, url)
        error_type can be: 'success', '404', 'timeout', 'encoding', 'network'
        url is the Software Heritage URL
    """
    # Construct Software Heritage URL
    url = SWH_API_BASE.format(blob_id=blob_id)
...
def process_batch_worker(args: Tuple) -> Tuple[List[Dict], Dict]:
    ...
    for sample in batch_metadata:
        ...
        # Download content from Software Heritage
        content, error_type, url = download_content_from_swh(blob_id)
...
def process_stack_dataset(
    ...
    for parquet_file in tqdm(parquet_files, desc="  📖 Reading metadata", unit=" files"):
        ...
        for batch in pf.iter_batches(batch_size=batch_size):
            ...
            # Extract metadata we need (blob_id)
            batch_metadata = []
    ...
    print(f"  Downloading content from Software Heritage with {num_workers} workers...")
...
def main():
    parser = argparse.ArgumentParser(description="Download and preprocess Stack v2 datasets from Software Heritage")
    ...
    parser.add_argument("--num_workers", type=int, default=None,
                       help="Number of worker processes (default: 4, capped to avoid rate limiting)")
    ...
    # Set default num_workers - cap at 4 for Software Heritage to avoid rate limiting
    if args.num_workers is None:
        args.num_workers = min(4, cpu_count())
        print(f"Using {args.num_workers} worker processes (capped for Software Heritage API)")
    ...
    print("="*80)
    print("Stack v2 Content Download and Preprocessing (Software Heritage)")
    print("="*80)
    ...
    print("="*80)
    print("ℹ️  Downloading from Software Heritage Archive")
    print("ℹ️  Live stats update every second during download")
    print("⚠️  Note: SWH has stricter rate limits - using fewer workers")
    print("="*80)
    ...
    # Save statistics
    stats_file = output_dir / "stack_v2_swh_preprocessing_stats.json"
    with open(stats_file, 'w') as f:
        json.dump(all_stats, f, indent=2)

    # Print summary
    print("\n" + "="*80)
    print("Stack v2 Preprocessing Summary (Software Heritage)")
    print("="*80)
