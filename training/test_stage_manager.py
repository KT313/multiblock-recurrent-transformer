...
def test_tiny_config_boundaries_by_hand() -> None:
    sm = StageManager(tiny_stages(), world_batch_size=4, training_max_sequence_length=256, warmup_steps=2, cooldown_steps=2)
...
def test_two_stage_config_boundaries_by_hand() -> None:
    sm = StageManager(two_stages(), world_batch_size=8, training_max_sequence_length=128)
...
def test_world_size_does_not_change_boundaries(stages: list[ResolvedStage], wbs: int, bs: int) -> None:
    one = StageManager(stages, world_batch_size=wbs, training_max_sequence_length=bs, world_size=1, micro_batch_size=1)
    four = StageManager(stages, world_batch_size=wbs, training_max_sequence_length=bs, world_size=4, micro_batch_size=1)
...
def test_world_batch_divisibility_validation() -> None:
    with pytest.raises(ValueError, match="divisible by world_size"):
        StageManager(tiny_stages(), world_batch_size=6, training_max_sequence_length=256, world_size=4)
    with pytest.raises(ValueError, match="multiple of micro_batch_size"):
        StageManager(tiny_stages(), world_batch_size=8, training_max_sequence_length=256, world_size=2, micro_batch_size=3)
    with pytest.raises(ValueError, match="at least one stage"):
        StageManager([], world_batch_size=4, training_max_sequence_length=256)
...
def test_warmup_or_cooldown_too_long_raises(warmup: int, cooldown: int) -> None:
    with pytest.raises(ValueError, match="must be less than"):
        StageManager(tiny_stages(), world_batch_size=4, training_max_sequence_length=256, warmup_steps=warmup, cooldown_steps=cooldown)
...
def test_warmup_and_cooldown_that_fit_are_accepted() -> None:
    ...
    sm = StageManager(tiny_stages(), world_batch_size=4, training_max_sequence_length=256, warmup_steps=5, cooldown_steps=3)
    assert sm.total_steps == 20
...
def test_a_stage_shorter_than_one_step_is_rejected() -> None:
    ...
    with pytest.raises(ValueError, match="stage 'b' is shorter than one optimizer step"):
        StageManager(stages, world_batch_size=4, training_max_sequence_length=256)
...
def test_get_stage_info_inside_and_outside_transitions() -> None:
    sm = StageManager(tiny_stages(), world_batch_size=4, training_max_sequence_length=256)
...
def test_data_weights_outside_a_transition_are_the_stage_constants() -> None:
    sm = StageManager(weighted_stages(), world_batch_size=4, training_max_sequence_length=256)
...
def test_tokens_per_step_replaces_the_sequence_product() -> None:
    ...
    packed = StageManager(tiny_stages(), world_batch_size=4, training_max_sequence_length=256, tokens_per_step=2048)
    ...
    same = StageManager(tiny_stages(), world_batch_size=4, training_max_sequence_length=256, tokens_per_step=1024)
    assert _bounds(same) == _bounds(StageManager(tiny_stages(), world_batch_size=4, training_max_sequence_length=256))
    with pytest.raises(ValueError, match="tokens_per_step must be positive"):
        StageManager(tiny_stages(), world_batch_size=4, training_max_sequence_length=256, tokens_per_step=0)
    with pytest.raises(ValueError, match="shorter than one optimizer step.*lower tokens_per_step"):
        StageManager(tiny_stages(), world_batch_size=4, training_max_sequence_length=256, tokens_per_step=5000)
