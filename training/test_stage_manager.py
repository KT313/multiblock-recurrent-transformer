...
def test_tiny_config_boundaries_by_hand() -> None:
    sm = StageManager(tiny_stages(), tokens_per_step=1024, warmup_steps=2, cooldown_steps=2)
...
def test_two_stage_config_boundaries_by_hand() -> None:
    sm = StageManager(two_stages(), tokens_per_step=1024)
...
def test_world_size_does_not_change_boundaries(stages: list[ResolvedStage], wbs: int, bs: int) -> None:
    one = StageManager(stages, tokens_per_step=wbs * bs, world_size=1)
    four = StageManager(stages, tokens_per_step=wbs * bs, world_size=4)
...
def test_constructor_validation() -> None:
    with pytest.raises(ValueError, match="tokens_per_step must be positive"):
        StageManager(tiny_stages(), tokens_per_step=0)
    with pytest.raises(ValueError, match="at least one stage"):
        StageManager([], tokens_per_step=1024)
...
def test_warmup_or_cooldown_too_long_raises(warmup: int, cooldown: int) -> None:
    with pytest.raises(ValueError, match="must be less than"):
        StageManager(tiny_stages(), tokens_per_step=1024, warmup_steps=warmup, cooldown_steps=cooldown)
...
def test_warmup_and_cooldown_that_fit_are_accepted() -> None:
    ...
    sm = StageManager(tiny_stages(), tokens_per_step=1024, warmup_steps=5, cooldown_steps=3)
    assert sm.total_steps == 20
...
def test_a_stage_shorter_than_one_step_is_rejected() -> None:
    ...
    with pytest.raises(ValueError, match="stage 'b' is shorter than one optimizer step"):
        StageManager(stages, tokens_per_step=1024)
...
def test_get_stage_info_inside_and_outside_transitions() -> None:
    sm = StageManager(tiny_stages(), tokens_per_step=1024)
...
def test_data_weights_outside_a_transition_are_the_stage_constants() -> None:
    sm = StageManager(weighted_stages(), tokens_per_step=1024)
...
def test_stage_summary_mentions_every_stage_and_step_counts() -> None:
    ...
    assert "Tokens per optimizer step: 1,024" in summary


def test_tokens_per_step_sizes_the_step() -> None:
    """
    A step is `tokens_per_step` tokens (`Settings.tokens_per_optimizer_step`): doubling it halves every boundary.
    """

    packed = StageManager(tiny_stages(), tokens_per_step=2048)
    assert packed.tokens_per_step == 2048
    ...
    assert "Tokens per optimizer step: 2,048" in packed.get_stage_summary()
    with pytest.raises(ValueError, match="shorter than one optimizer step.*lower tokens_per_micro_batch or micro_batches_per_step"):
        StageManager(tiny_stages(), tokens_per_step=5000)
