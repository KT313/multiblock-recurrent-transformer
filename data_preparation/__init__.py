...
repo root. ``dataset_config.py`` (the config schema and reference) and ``layout.py`` (paths under ``dataset/``) live
here; the rest is in ``lib/``: ``sources`` (loaders, converters), ``stages`` (pipeline steps), ``storage``
