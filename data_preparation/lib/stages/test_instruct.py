...
def _build(
    ...
    mixture: MixtureConfig, rows_needed: int = 40, **kw: Any,
) -> DatasetConfig:  # fmt: skip
    cfg = with_tokenizer(cfg_factory(sources, mixtures={"m": mixture}, **kw))
    for name in sources:
        download(cfg, name, layout, rows_needed=rows_needed, shard_size=16)
    return cfg


def test_build_mixture_counts_split_and_columns(
    ...
    mixture = MixtureConfig(sources={"a": 0.6, "b": 0.4}, max_tokens=64, input_inversions=0.0, val_split=0.25, seed=1)
    cfg = _build(cfg_factory, layout, with_tokenizer, two_sources, mixture)
    result = build_mixture(cfg, "m", layout, budget_tokens=200, shard_size=8)
    train, val = result["train"], result["validation"]
    assert train.stage == val.stage == "mixture" and train.source_hash == cfg.mixture_hash("m")
    ...
    rows = read_rows(layout.mixture_dir("t", "m", "train")) + read_rows(layout.mixture_dir("t", "m", "validation"))
    ...
    assert train.tokens() == sum(r["tokens"] for r in read_rows(layout.mixture_dir("t", "m", "train")))
    ...
    before = {split: mtimes(layout.mixture_dir("t", "m", split)) for split in result}
    again = build_mixture(cfg, "m", layout, budget_tokens=200, shard_size=8)
    assert again == result
    assert {split: mtimes(layout.mixture_dir("t", "m", split)) for split in result} == before
    assert Manifest.load(layout.mixture_dir("t", "m", "validation")) == val


def test_build_mixture_short_sources_and_length_check(
    cfg_factory: CfgFactory, layout: DatasetLayout, with_tokenizer: Prep, two_sources: dict[str, SourceConfig], read_rows: Reader
) -> None:
    mixture = MixtureConfig(sources={"a": 0.5, "b": 0.5}, max_tokens=6, val_split=0.0, seed=0)
    cfg = _build(cfg_factory, layout, with_tokenizer, two_sources, mixture, rows_needed=5)
    result = build_mixture(cfg, "m", layout, budget_tokens=600)
    train = result["train"]
    ...
    assert all(r["tokens"] == 6 for r in read_rows(layout.mixture_dir("t", "m", "train")))


def test_build_mixture_inversions_dedup_and_empty_removal(
    ...
    mixture = MixtureConfig(sources={"c": 1.0}, max_tokens=64, input_inversions=0.5, val_split=0.0, seed=3)
    cfg = _build(cfg_factory, layout, with_tokenizer, {"c": src}, mixture)
    train = build_mixture(cfg, "m", layout, budget_tokens=10_000)["train"]
    meta = train.extra["metadata"]
    out = read_rows(layout.mixture_dir("t", "m", "train"))
...
def test_build_mixture_rebuilds_on_new_budget_hash_or_input_shards(
    cfg_factory: CfgFactory, layout: DatasetLayout, with_tokenizer: Prep, two_sources: dict[str, SourceConfig], mtimes: Mtimes
) -> None:
    mixture = MixtureConfig(sources={"a": 0.5, "b": 0.5}, max_tokens=64, val_split=0.2, seed=0)
    cfg = _build(cfg_factory, layout, with_tokenizer, two_sources, mixture, rows_needed=10)
    train_dir = layout.mixture_dir("t", "m", "train")
    first = build_mixture(cfg, "m", layout, budget_tokens=100)["train"]
    t0 = mtimes(train_dir)
    second = build_mixture(cfg, "m", layout, budget_tokens=120)["train"]
    ...
    third = build_mixture(cfg, "m", layout, budget_tokens=120)["train"]
    ...
    reseeded = _build(cfg_factory, layout, with_tokenizer, two_sources,
                      MixtureConfig(sources={"a": 0.5, "b": 0.5}, max_tokens=64, val_split=0.2, seed=7), rows_needed=20)  # fmt: skip
    fourth = build_mixture(reseeded, "m", layout, budget_tokens=120)["train"]
    assert fourth.source_hash == reseeded.mixture_hash("m") != third.source_hash


def test_build_mixture_requires_raw_manifests(cfg_factory: CfgFactory, layout: DatasetLayout, two_sources: dict[str, SourceConfig]) -> None:
    mixture = MixtureConfig(sources={"a": 1.0})
    cfg = cfg_factory({"a": two_sources["a"]}, mixtures={"m": mixture})
    with pytest.raises(FileNotFoundError, match="no current raw manifest"):
        build_mixture(cfg, "m", layout, budget_tokens=10)
