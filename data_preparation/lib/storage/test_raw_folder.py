...
def test_the_folder_reads_the_manifest_fields(tmp_path: Path) -> None:
    manifest = _manifest()
    assert not manifest.exhausted and manifest.check_limit_reached is None and (manifest.skipped_malformed, manifest.dropped_too_long) == (0, 0)
    manifest.exhausted, manifest.check_limit_reached, manifest.skipped_malformed, manifest.dropped_too_long = True, 5, 3, 2
    assert manifest.exhausted and manifest.check_limit_reached == 5 and (manifest.skipped_malformed, manifest.dropped_too_long) == (3, 2)
...
def test_mark_exhausted_records_the_limit_and_saves(tmp_path: Path) -> None:
    ...
    assert stored is not None and stored.exhausted and stored.check_limit_reached == 7
...
def test_appending_records_offset_and_reject_counters_per_shard(tmp_path: Path) -> None:
    ...
    assert stored.rows_fetched == 112 and (stored.skipped_malformed, stored.dropped_too_long) == (9, 4) and not stored.exhausted
...
def test_finish_marks_exhaustion_with_the_limit_that_caused_it(tmp_path: Path) -> None:
    ...
    assert folder.exhausted and folder.manifest.check_limit_reached == 9
...
def test_truncate_to_good_prefix_restores_offset_and_every_counter(tmp_path: Path) -> None:
    ...
    manifest = _manifest(rows_fetched=30, exhausted=True, check_limit_reached=30, skipped_malformed=9, dropped_too_long=6)
...
def test_truncate_to_good_prefix_of_a_manifest_without_counters_resets_them(tmp_path: Path, caplog: pytest.LogCaptureFixture) -> None:
    ...
    manifest = _manifest(rows_fetched=20, skipped_malformed=9, dropped_too_long=6)
