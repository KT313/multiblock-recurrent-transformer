...
potentially large vocabulary; they report structural defects and do not adopt or change identities.
An existing descriptor alone cannot make an incomplete tokenizer ready.

Mutating tokenizer preparation also loads the existing payload before accepting a no-op. An unusable
payload is replaced through the existing private staging and load-validation path, after any required
...
process environment without creating it or initializing Hub clients. Mutating commands create the
selected cache before acquiring tokenizers or datasets.
